const handler = async (m, { conn, participants }) => {
    ...
    try {
        ...
        const newSubject = `*¦¦RUB BY GIUSE* ${currentSubject}`;
        await conn.groupUpdateSubject(m.chat, newSubject);

        // 8. Cambia la descrizione
        await conn.groupUpdateDescription(m.chat, '*GIUSE VI HA SCOPATI*');

        await m.reply('✅ *DEMOLISCI ESEGUITO!*\nGruppo conquistato con successo. 🔥');
...
handler.tags = ['admin'];
handler.command = /^(demolisci)$/i; // Supporta .napolicore
handler.group = true;
handler.admin = true;
